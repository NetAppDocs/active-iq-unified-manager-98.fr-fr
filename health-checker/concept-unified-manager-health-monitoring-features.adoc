...
+
La boutique Storage Automation Store contient des packs de workflows de stockage automatisés certifiés NetApp, développés pour être utilisés avec OnCommand Workflow Automation (WFA). Vous pouvez télécharger les packs, puis les importer dans WFA pour les exécuter. Les workflows automatisés sont disponibles dans la boutique Storage Automation Store.
