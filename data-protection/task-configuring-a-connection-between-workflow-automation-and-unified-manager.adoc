...
[NOTE]
====
Le « pack WFA pour la gestion de Clustered Data ONTAP » est inclus dans WFA 5.1, il n’est donc plus nécessaire de télécharger ce pack depuis le NetAppStorage Automation Store et de l’installer séparément sur votre serveur WFA comme c’était le cas auparavant.  https://community.netapp.com/["WFA pack pour la gestion de ONTAP"^]
